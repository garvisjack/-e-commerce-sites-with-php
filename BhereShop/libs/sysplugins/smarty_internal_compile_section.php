...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
     ...
     * @see Smarty_Internal_CompileBase
     */
    public $optional_attributes = array('start', 'step', 'max', 'select', 'properties');
     ...
     * @var array
     */
    public $nameProperties = array('first', 'last', 'index', 'iteration', 'select', 'total', 'rownum', 'index_prev',
                                   'index_next', 'loop');
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler)
    {
        ...
        $incFor = array();
        $cmpFor = array();
        $propValue = array('index' => "{$sectionVar}->value['index']", 'select' => 'true', 'step' => 1,
                           'iteration' => "{$local}iteration",

        );
        $propType = array('index' => 2, 'iteration' => 2, 'select' => 0, 'step' => 0,);
        ...
        if ($propType[ 'select' ] > 0) {
            $output .= "{$local}select = {$propValue['select']} ? {$cond_code} : false;\n";
            $output .= "if ({$local}select) {\n";
        } elseif ($propValue[ 'select' ] == 'true') {
            $output .= "if ({$cond_code}) {\n";
        } else {
            $output .= "if (false) {\n";
        }
